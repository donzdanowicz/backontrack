package com.backontrack.stream.portfolio;
...
//        List<Integer> daysList = project.getTaskLists().stream()
//                .filter(tl -> tl.getName().equals("In progress"))
//                .flatMap(tl -> tl.getTasks().stream())
//                .map(Task::getCreated)
//                .map(t -> Period.between(t, LocalDate.now()).getDays())
//                        .collect(toList());
//
//        OptionalDouble average = IntStream.range(0, daysList.size())
//                .average();

        double average = days / count;
